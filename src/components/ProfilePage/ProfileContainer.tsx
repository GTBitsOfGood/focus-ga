...
const ProfileHeader = ({ user, currUser, openModal, setShowBanDialog, isBanned, banLoading }: any) => (
  <div className="flex flex-row mb-6 items-start justify-between">
    <div className="flex flex-row space-x-6">
      <div className={`flex items-center justify-center w-[108px] h-[108px] rounded-full bg-${user.profileColor} relative`}>
        ...
        {user._id === currUser._id && <ColorPicker handleColorPick={openModal} />}
      </div>
    ...
    {user._id === currUser._id ? (
      <button onClick={openModal} className="bg-light-gray hover:bg-zinc-300 transition text-theme-gray text-lg font-bold px-4 py-2 rounded-lg">
  ...
  <div>
    ...
    <div className="flex flex-row mb-4">
      ...
      <div className={`flex flex-row flex-wrap gap-3 ${user.childDisabilities.length > 0 ? 'py-1' : '-my-1'}`}>
        {user.childDisabilities.map((disability: any, index: number) => (
          <Tag text={disability.name} key={index} />
        ))}
      </div>
...
const ProfileTabs = ({ user, currUser, userPosts, savedPosts }: any) => (
    ...
    <div>
      ...
      <div className="mx-16 my-4">
        <div className="mx-14 mt-8">
          ...
          <AlertDialog open={showBanDialog}>
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                >
                  {isBanned ? (banLoading ? 'Unbanning...' : 'Unban') : (banLoading ? 'Banning...' : 'Ban')}
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
          ...
          <ProfileTabs user={user} currUser={currUser} userPosts={userPosts} savedPosts={savedPosts} />
        </div>
      </div>
    </div>
  );
